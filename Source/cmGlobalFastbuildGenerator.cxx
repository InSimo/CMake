...
complaints and fails generation? a re-run will succeed.
         - Linker for msvc uses the cmake command to callback. Don't think this
is an issue since I think compilation is the part that gets distributed anyway.
         But it might mean that the cache has trouble calculating deps for
obj->lib/exe. Not sure if Fastbuild supports that anyway yet
         - Need to sort custom build commands by their outputs
         - Unit tests BuildDepends, RunCMake.Configure, and requires auto-rerun
of cmake when detecting changes to source scripts. Not implementable unless
FASTBuild can detect when a rule outputs the current bff file and reload it
afterwards ?
         - Unit test SimpleInstall (and SimpleInstall-Stage2) call cmake
--build . as post-build commands, leading to FASTBuild to exit with error
...
output when that target is actually defined. Rather than originally complaining
that the target doesn't exist where the reference is attempted.
         - Parsing strings with double $$ doesn't generate a nice error
         - Undocumented that you can escape a $ with ^$
           -> now documented in
http://www.fastbuild.org/docs/syntaxguide.html#escaping
...
then cycles would be possible
         ...
         - Issues with preserving escaped quotes and double spaces, such as in
-DMYVAR="\"string  with 2 spaces\""
           -> disabled it from Preprocess unit test, but it would be cleaner to
find a way to fix it instead
...
#define FASTBUILD_DOLLAR_TAG "FASTBUILD_DOLLAR_TAG"
  ...
  static void DetectConfigurations(
    ...
    std::map<std::string, std::string>& configAlias)
  {
    ...
    if (configList) {
      ...
      cmExpandList(configList, argsOut);
  ...
  {
    cmGeneratorTarget::Names targetName;
                           ...
                           const std::string& configName)
  {
    if (target.GetType() == cmStateEnums::EXECUTABLE) {
      targetNamesOut.targetName = target.GetExecutableNames(configName);
    } else {
      targetNamesOut.targetName = target.GetLibraryNames(configName);
    }
    ...
    if (target.HaveWellDefinedOutputFiles()) {
      ...
      targetNamesOut.targetOutputReal =
        target.GetFullPath(configName, cmStateEnums::RuntimeBinaryArtifact,
                           /*realpath=*/true);
      targetNamesOut.targetOutputImplib =
        target.GetFullPath(configName, cmStateEnums::ImportLibraryArtifact);
    } else {
      targetNamesOut.targetOutputDir =
        target.Target->GetMakefile()->GetCurrentBinaryDirectory();
      ...
      targetNamesOut.targetOutput =
        targetNamesOut.targetOutputDir + "/" + targetNamesOut.targetName.Output;
      targetNamesOut.targetOutputImplib = targetNamesOut.targetOutputDir +
        "/" + targetNamesOut.targetName.ImportLibrary;
      targetNamesOut.targetOutputReal =
        targetNamesOut.targetOutputDir + "/" + targetNamesOut.targetName.Real;
    }
    ...
    if (target.GetType() <= cmStateEnums::OBJECT_LIBRARY) {
      ...
      if (targetNamesOut.targetOutputCompilePDBDir.empty()) {
        targetNamesOut.targetOutputCompilePDBDir =
          target.GetSupportDirectory() + "/" + configName + "/";
      }
    }
    ...
    switch (target.GetType()) {
      case cmStateEnums::STATIC_LIBRARY: {
        ...
        // TODO: Use ARCHIVE_APPEND for archives over a certain size.
        {
          ...
          std::string linkCmd = mf->GetRequiredDefinition(linkCmdVar);
          linkCmds.clear();
          cmExpandList(linkCmd, linkCmds);
        }
        {
                                    ...
                                    const std::string& language)
  {
    ...
    //lg->AddCompileDefinitions(defines, &target, configName, language); // TODO: still needed ?

    if (source) {
      cmGeneratorExpressionInterpreter genexInterpreter(
        lg, configName, &target, language);

      const std::string COMPILE_DEFINITIONS("COMPILE_DEFINITIONS");
      if (const char* compile_defs =
                                   ...
                                   const std::string& configName)
  {
    ...
    for (std::vector<std::string>::const_iterator libDir = libDirs.begin();
      ...
      std::string libpath =
        lg->ConvertToOutputForExisting(*libDir, cmOutputConverter::SHELL);
                                      ...
                                      const std::string& configName)
  {
    ...
    cmGlobalFastbuildGenerator* gg =
      static_cast<cmGlobalFastbuildGenerator*>(lg->GetGlobalGenerator());
    const std::string manifests =
      gg->GetManifestsAsFastbuildPath(target, configName);
    ...
    vars.ObjectDir =
      FASTBUILD_DOLLAR_TAG "TargetOutCompilePDBDir" FASTBUILD_DOLLAR_TAG;
    vars.Target =
    ...
    std::string launcher;
    const char* val = lg->GetRuleLauncher(&target, "RULE_LAUNCH_LINK");
    if (val && *val) {
      launcher = val;
      launcher += " ";
    }
                              ...
                              cmGeneratorTarget& target)
  {
    ...
    // nor utilities
    bool hasObjectGroups = target.GetType() != cmStateEnums::UTILITY &&
  ...
  static void DetectCompilerFlags(std::string& compileFlags,
                                  ...
                                  const std::string& configName)
  {
    lg->AddLanguageFlags(compileFlags, &target, language, configName);

    lg->AddArchitectureFlags(compileFlags, &target, language, configName);
  ...
  static void DetectTargetCompileDependencies(
    cmGlobalFastbuildGenerator* gg, cmGeneratorTarget& target,
    ...
    std::vector<std::string>& linkDependencies)
  {
    if (target.GetType() == cmStateEnums::GLOBAL_TARGET) {
      ...
      auto& utils = target.GetUtilities();
      for (const auto& x : utils) {
        dependencies.push_back(x.Value);
      }
    } else {
  ...
  /*
          static void DetectTargetLinkDependencies(
                  ...
                  std::vector<std::string>& dependencies)
          {
                  // Static libraries never depend on other targets for
     linking. if (target.GetType() == cmStateEnums::STATIC_LIBRARY ||
  ...
  */
  ...
  static void DetectTargetLinkItems(cmGeneratorTarget& target,
                                    const std::string& configName,
                                    ...
                                    std::ostringstream& log)
  {
    ...
    for (cmComputeLinkInformation::ItemVector::const_iterator li =
      ...
      if (li->IsPath)
        log << " (Path)";
      if (li->Target)
        log << " (Target " << li->Target->GetName() << " "
            << cmState::GetTargetTypeName(li->Target->GetType()) << ")";
      log << "\n";
      if (li->Target &&
      ...
      if (li->IsPath || li->Target) {
        libs.push_back(
          li->Value); // += this->ConvertToLinkReference(li->Value,
                      // shellFormat);
      }
      // else {
  ...
  static std::string DetectTargetCompileOutputDir(
    cmLocalFastbuildGenerator* lg, const cmGeneratorTarget& target,
    std::string configName)
  {
  ...
  static void DetectCustomCommandOutputs(
    const cmCustomCommand* cc, cmGlobalFastbuildGenerator* gg,
    cmLocalFastbuildGenerator* lg, cmGeneratorTarget& target,
    const std::string& configName, std::vector<std::string>& fileOutputs,
    std::vector<std::string>& symbolicOutputs, bool& isConfigDependant,
    std::ostringstream* outDescription = NULL)
  {
    ...
    // Check if the outputs don't depend on the config name, and if we have a
    // real output
    for (std::vector<std::string>::iterator iter = outputs.begin();
         iter != outputs.end(); ++iter) {
      std::string dep = *iter;
      cmSourceFile* depSourceFile = makefile->GetSource(
        dep); // GetSource called before any filtering of the file path
      bool isSymbolic =
        depSourceFile && depSourceFile->GetPropertyAsBool("SYMBOLIC");
      ...
      // Check if this file is symbolic and if it depends on config value
      // (before resolving $ConfigName$)
      bool isConfigDep = dep.find("$ConfigName$") != std::string::npos;
      isConfigDependant = isConfigDependant || isConfigDep;
      if (outDescription)
        (*outDescription) << "        - " << dep
                          << (depTarget ? " (Target)" : "")
                          << (depSourceFile ? " (SourceFile)" : "")
                          << (isSymbolic ? " (SYMBOLIC)" : "")
                          << (isConfigDep ? " (CONFIG)" : "") << "\n";
  ...
  struct DependencySorter
  {
    struct TargetHelper
    {
      ...
      void AddDependency(const cmGeneratorTarget* /*from*/,
                         const cmGeneratorTarget* /*to*/)
      {
      }
    };
    ...
    struct CustomCommandHelper
    {
      ...
      std::map<const cmSourceFile*, std::vector<std::string>>
        mapSymbolicOutputs;
      std::map<const cmSourceFile*, std::vector<const cmSourceFile*>>
        mapInputCmds;
                      ...
                      std::vector<std::string>& outputs)
      {
        ...
        outputs.insert(outputs.end(), mapSymbolicOutputs[entry].begin(),
                       mapSymbolicOutputs[entry].end());
      }
    ...
    static void Sort(TTypeHelper& helper, std::vector<const TType*>& entries)
    {
      ...
      std::sort(entries.begin(), entries.end(),
                SorterFunctor<TType, TTypeHelper>(&helper));
      ...
      for (EntryIndex index = 0; index < entries.size(); ++index) {
        ...
        for (StringVector::const_iterator inIter = inputs.begin();
          ...
          if (findResult != outputMap.end()) {
            ...
            if (std::find(entryInputs.begin(), entryInputs.end(),
                          dentryIndex) == entryInputs.end()) {
      ...
      while (!forwardDeps.empty() && written) {
        ...
        for (typename DepMap::iterator iter = forwardDeps.begin();
          ...
          for (unsigned int i = 0; i < revDeps.size(); ++i) {
            ...
            std::vector<EntryIndex>& revDepFwdDeps = forwardDeps[revDepIndex];
  ...
  struct RemovalTest
  {
    bool operator()(const cmGeneratorTarget* target) const
    {
      if (target->GetType() == cmStateEnums::GLOBAL_TARGET) {
        ...
        if (strcmp(mf->GetCurrentSourceDirectory().c_str(),
                   mf->GetCMakeInstance()->GetHomeDirectory().c_str()) != 0) {
          // std::ostringstream s;
          // s << "Ignoring global target " << target->GetName() << " as it is
          // not from the top-level directory: " <<
          // mf->GetCurrentSourceDirectory() << " != " <<
          // mf->GetCMakeInstance()->GetHomeDirectory() << std::endl;
                                       ...
                                       std::vector<std::string>& extraFiles)
  {
    ...
    if (compilerID == "MSVC") {
      ...
      std::string compilerPath = cmSystemTools::GetFilenamePath(path);
      ...
      const std::string prefixBinMS =
        inSubdir ? prefixBinCL + "..\\" : prefixBinCL;
      const std::string prefixBinUI = prefixBinCL + "1033\\";
      const std::string prefixBinCRT = clIsX64
        ? prefixBinMS + "..\\redist\\x64\\"
        : prefixBinMS + "..\\redist\\x86\\";
      ...
      if (version.compare(0, 3, "19.") != std::string::npos) {
        ...
        extraFiles.insert(extraFiles.end(), &vs2015_extraFiles[0],
                          &vs2015_extraFiles[12]);
        ...
        std::string vs2012_extraFiles[12] = {
          ...
          prefixBinUI + "clui.dll",
          "$CompilerRoot$\\..\\..\\redist\\x86\\Microsoft.VC110.CRT\\msvcp110."
          "dll",
        ...
        extraFiles.insert(extraFiles.end(), &vs2010_extraFiles[0],
                          &vs2010_extraFiles[11]);
      }
    }
  }
...
public:
                                  ...
                                  TargetContextMap& map)
  {
    ...
    for (; it != end; ++it) {
      ...
      for (std::vector<cmLocalGenerator*>::const_iterator iter =
        ...
        auto& tgts = lg->GetMakefile()->GetTargets();
        for (auto targetIter = tgts.begin();
          ...
          cmGeneratorTarget* gt =
            lg->FindGeneratorTargetToUse(target.GetName());
          ...
          // std::ostringstream s;
          // s << "Creating context for target " << target.GetName() << " in "
          // << lg->GetCurrentSourceDirectory() << " ( 0x" << std::hex <<
          // (unsigned long long) gt << std::dec << " ) " << " ( 0x" << std::hex
          // << (unsigned long long) gt->Target << std::dec << " ) " <<
          // std::endl; cmSystemTools::Message(s.str().c_str());
  ...
  static void WriteCustomCommand(
    ...
    cmLocalFastbuildGenerator* lg, cmGeneratorTarget& target,
    const std::string& configName, const std::vector<std::string>& fileOutputs,
    const std::vector<std::string>& symbolicOutputs, std::string& targetName,
    const std::string& hostTargetName, bool isConfigDependant,
    const std::vector<std::string>& deps)
  {
    ...
    std::string command0 =
      ccg.GetNumberOfCommands() > 0 ? ccg.GetCommand(0) : std::string();
    Detection::UnescapeFastbuildVariables(command0);
    const cmGeneratorTarget* command0target =
      ccg.GetNumberOfCommands() > 0 ? ccg.GetCommandTarget(0) : NULL;
    ...
    context.fc.WriteComment(std::string("Command0:   ") + command0 +
                            (command0target ? std::string(" ( ") +
                                 command0target->GetName() + std::string(" ) ")
                                            : std::string()));
    context.fc.WriteCommentMultiLines(std::string("Depends: ") +
                                      Join(depends, "\n         "));
    ...
    if (!fileOutputs.empty()) {
      ...
      if (isConfigDependant)
        firstOutputOrSymbolic += "-" + configName;
    }
    ...
    if (!firstOutputOrSymbolic.empty()) {
      ...
      CustomCommandAliasMap::iterator findResult =
        context.customCommandAliases.find(
          std::make_pair(cc, firstOutputOrSymbolic));
      if (findResult != context.customCommandAliases.end()) {
        const std::vector<std::string>& aliases = findResult->second;
        if (std::find(aliases.begin(), aliases.end(), targetName) !=
            aliases.end()) {
          context.fc.WriteComment(targetName + ": " + firstOutputOrSymbolic +
                                  " already defined");
        ...
        if (!aliases.empty()) {
          context.fc.WriteComment(targetName + ": " + firstOutputOrSymbolic +
                                  " already defined as " + aliases.front());
          ...
          // This might create a dependency between different configurations
          // (i.e. Debug target being compiled when asking for Release)
          // TODO: We should output a warning about this here...
          // targetName += "-";
          // targetName += hostTargetName;
          /*
                                                  std::vector<std::string>
             targets; targets.push_back(*findResult->second.begin());

                                                  context.fc.WriteCommand("Alias",
             Quote(targetName)); context.fc.WritePushScope();
                                                  {
      ...
      context.customCommandAliases[std::make_pair(cc, firstOutputOrSymbolic)]
        .push_back(targetName);
    } else {
      // No output, so this command must always be run.
      // Make it's name unique to its host target
      // -> no longer necessary, the targetName is already prefixed by the
      // actual target to eliminate duplicates when the same filename is
      // present in different targets/subdirs
      // targetName += "-";
      // targetName += hostTargetName;
    }
    ...
    for (unsigned int ci = 0; ci < ccg.GetNumberOfCommands(); ++ci) {
      ...
      if (cmdtarget != NULL && !cmdtarget->IsImported() &&
          cmdtarget->Target != target.Target) {
    ...
    // FIX: workindDirectory may be within the source hierarchy that we are not
    // permitted to write to so the script is now always generated in the
    ...
    workingDirectory =
      target.LocalGenerator->GetGlobalGenerator()->ExpandCFGIntDir(
    ...
    this Exec(Rarely needed, ; but useful when Exec relies on externally
    generated files).
    }
    */
    ...
    context.fc.WritePushScope();
    {
      ...
      if (hasSymbolicOutput &&
          inputs.empty()) // TODO: probably not the best criteria, works in our
                          // use cases...
      {
        context.fc.WriteVariable(
          "ExecAlwaysRun",
          "true"); // Requires modified version of FASTBuild (after 0.90)
      }
      if (!orderDependencies.empty()) {
        context.fc.WriteArray("PreBuildDependencies", Wrap(orderDependencies),
                              "+"); // , (configName == "Common") ? "=" : "+");
      }
    }
    context.fc.WritePopScope();
  ...
  static void WriteCustomBuildSteps(
    ...
    cmGeneratorTarget& target, const std::vector<cmCustomCommand>& commands,
    ...
    const std::vector<std::string>& commonDeps = std::vector<std::string>())
  {
    ...
    for (; iter != end; ++iter) {
      ...
      for (std::vector<cmCustomCommand>::const_iterator ccIter =
        ...
        Detection::DetectCustomCommandOutputs(
          &cc, context.self, lg, target, configName, fileOutputs,
          symbolicOutputs, isConfigDependant);
        std::string customCommandTargetNameStr = customCommandTargetName.str();
        WriteCustomCommand(context, &cc, lg, target, configName, fileOutputs,
                           symbolicOutputs, customCommandTargetNameStr,
    ...
    cmGeneratorTarget& target)
  {
    ...
    const std::vector<std::string>& configs =
      context.self->GetConfigurations();
    ...
    // compute inputs, outputs, and working dir for each config-command
    // combination
    cmMakefile* makefile = lg->GetMakefile();
    ...
    std::map<std::string, Detection::DependencySorter::CustomCommandHelper>
      mapConfigCC;
    std::set<const cmSourceFile*> setConfigDependant;

    for (std::vector<cmSourceFile const*>::const_iterator ccIter =
           allCustomCommands.begin();
      ...
      for (std::vector<std::string>::const_iterator cfIter = configs.begin();
           cfIter != configs.end(); ++cfIter, firstConfig = false) {
        ...
        std::string& workingDirectory =
          mapConfigCC[configName].mapWorkingDirectory[sourceFile];
        std::vector<std::string>& inputs =
          mapConfigCC[configName].mapInputs[sourceFile];
        std::vector<std::string>& fileOutputs =
          mapConfigCC[configName].mapFileOutputs[sourceFile];
        std::vector<std::string>& symbolicOutputs =
          mapConfigCC[configName].mapSymbolicOutputs[sourceFile];
        ...
        for (std::vector<std::string>::const_iterator iter = depends.begin();
          ...
          if (firstConfig)
            description
              << "        - " << dep
              << (depTarget ? std::string(" (Target") +
                      cmState::GetTargetTypeName(depTarget->GetType()) + ")"
                            : std::string())
              << (depSourceFile ? " (SourceFile)" : "")
              << (isSymbolic ? " (SYMBOLIC)" : "") << "\n";
          if (depTarget != NULL) {
            // inputTargets.push_back(dep);
            // FIX: disable this condition, as it breaks OutDir unit test
            // (using a target executable to generate a header) instead we keep
            // track of this list of target and add it as prebuilddeps
            // isConfigDependant = true;
        ...
        for (unsigned int ci = 0; ci < ccg.GetNumberOfCommands(); ++ci) {
          ...
          if (target != NULL) {
            ...
            if (!target->IsImported()) // we ignore imported targets as they
                                       // are probably not dependant on config
                                       // (such as Qt4::moc)
            {
              if (firstConfig)
                description << "        - command \"" << target->GetName()
        ...
        // Find outputs and see if they don't depend on the config name, and if
        // we have a real output
        if (firstConfig)
          description << "    Outputs:\n";
        Detection::DetectCustomCommandOutputs(
          cc, context.self, lg, target, configName, fileOutputs,
          symbolicOutputs, isConfigDependant,
          firstConfig ? &description : NULL);
      }
      if (!isConfigDependant) {
        const std::string refConfigName = configs.front();
        const std::string& refWorkingDirectory =
          mapConfigCC[refConfigName].mapWorkingDirectory[sourceFile];
        const std::vector<std::string>& refInputs =
          mapConfigCC[refConfigName].mapInputs[sourceFile];
        const std::vector<std::string>& refFileOutputs =
          mapConfigCC[refConfigName].mapFileOutputs[sourceFile];
        const std::vector<std::string>& refSymbolicOutputs =
          mapConfigCC[refConfigName].mapSymbolicOutputs[sourceFile];

        for (std::vector<std::string>::const_iterator cfIter =
               ++configs.begin();
             cfIter != configs.end(); ++cfIter) {
          const std::string& configName = *cfIter;
          ...
          const std::vector<std::string>& symbolicOutputs =
            mapConfigCC[configName].mapSymbolicOutputs[sourceFile];
          if (workingDirectory != refWorkingDirectory ||
              inputs.size() != refInputs.size() ||
              fileOutputs.size() != refFileOutputs.size() ||
              symbolicOutputs.size() != refSymbolicOutputs.size()) {
            description << " - mismatch in workingDirectory (\""
                        << workingDirectory << "\"!=\"" << refWorkingDirectory
                        << "\") or number of inputs (" << inputs.size()
                        << "!=" << refInputs.size()
                        << ") or number of outputs (" << fileOutputs.size()
                        << "!=" << refFileOutputs.size() << " or "
                        << symbolicOutputs.size()
                        << "!=" << refSymbolicOutputs.size() << ")\n";
            isConfigDependant = true;
            break;
          }
          for (std::size_t index = 0; index < inputs.size(); ++index) {
            if (inputs[index] != refInputs[index]) {
              description << " - mismatch in an input (\"" << inputs[index]
                          << "\"!=\"" << refInputs[index] << "\")\n";
              isConfigDependant = true;
              break;
            }
          ...
          for (std::size_t index = 0; index < fileOutputs.size(); ++index) {
            if (fileOutputs[index] != refFileOutputs[index]) {
              description << " - mismatch in a file output (\""
                          << fileOutputs[index] << "\"!=\""
                          << refFileOutputs[index] << "\")\n";
              isConfigDependant = true;
              break;
            }
          }
          ...
          for (std::size_t index = 0; index < symbolicOutputs.size();
               ++index) {
            if (symbolicOutputs[index] != refSymbolicOutputs[index]) {
              description << " - mismatch in a symbolic output (\""
                          << symbolicOutputs[index] << "\"!=\""
                          << refSymbolicOutputs[index] << "\")\n";
              isConfigDependant = true;
              break;
            }
          }
    ...
    if (!allCustomCommands.empty()) {
      ...
      const Detection::DependencySorter::CustomCommandHelper& cch0 =
        mapConfigCC[firstConfigName];
      ...
      for (; iter != end; ++iter, firstConfig = false) {
        ...
        Detection::DependencySorter::CustomCommandHelper& cch =
          mapConfigCC[configName];

        std::vector<const cmSourceFile*>& configCustomCommands =
          cch.orderedCommands;
        // configCustomCommands = allCustomCommands;
        // copy the commands that could be shared between configs
        for (std::vector<const cmSourceFile*>::const_iterator it =
               allCustomCommands.begin();
             it != allCustomCommands.end(); ++it) {
        ...
        {
          for (std::vector<const cmSourceFile*>::const_iterator it =
                 configCustomCommands.begin();
               it != configCustomCommands.end(); ++it) {
            if (!setConfigDependant.count(*it)) {
              ++countCommonCommands;
            } else {
              break;
        ...
        for (unsigned int index = 0; index < configCustomCommands.size();
             ++index) {
          ...
          if ((firstConfig && index == 0) ||
              index == countCommonCommands) // begin a new block of commands
          {
            customCommandTargets.clear();
            commandNameBase =
              targetName + "-CustomCommand-" + activeConfigName + "-";
            commandGroupName =
              targetName + "-CustomCommands-" + activeConfigName;
            context.fc.WriteVariable("CustomCommands_" + activeConfigName, "");
            context.fc.WritePushScopeStruct();
            context.fc.WriteCommand("Using",
                                    ".BaseConfig_" + activeConfigName);
          }

          if (!firstConfig && isCommon) {
            // no need to output it again, just copy its ref
            cch.mapRuleName[sourceFile] =
              cch0.mapRuleName.find(sourceFile)->second;
          } else {
            ...
            const std::vector<std::string>& fileOutputs =
              cch.mapFileOutputs[sourceFile];
            const std::vector<std::string>& symbolicOutputs =
              cch.mapSymbolicOutputs[sourceFile];
            const std::vector<const cmSourceFile*>& inputCmds =
              cch.mapInputCmds[sourceFile];
            std::vector<std::string> commandDeps;
            for (std::vector<const cmSourceFile*>::const_iterator it =
                   inputCmds.begin();
                 it != inputCmds.end(); ++it) {
              // context.fc.WriteComment("Input Dependency on command " +
              // (*it)->GetFullPath()); if (std::find(inputs.begin(),
              // inputs.end(), output0) == inputs.end())
              {
            ...
            WriteCustomCommand(context, sourceFile->GetCustomCommand(), lg,
                               target, configName, fileOutputs,
                               symbolicOutputs, commandTargetNameStr,
          ...
          if ((index >= countCommonCommands &&
               index == configCustomCommands.size() - 1) ||
              (firstConfig &&
               index == countCommonCommands - 1)) // end of block of commands
          {
            // Write an alias for this object group to group them all together
                                    ...
                                    cmGeneratorTarget& target)
  {
    ...
    switch (target.GetType()) {
      case cmStateEnums::INTERFACE_LIBRARY:
        ...
        // context.fc.WriteComment(std::string("Target definition: ") +
        // target.GetName() + " (" +
        // cmState::GetTargetTypeName(target.GetType()) + ") IGNORED");
        return;
      case cmStateEnums::EXECUTABLE: {
    ...
    // Object libraries do not have linker stages
    // nor utilities
    bool hasLinkerStage = target.GetType() != cmStateEnums::OBJECT_LIBRARY &&
      target.GetType() != cmStateEnums::UTILITY &&
      target.GetType() != cmStateEnums::GLOBAL_TARGET;

    bool hasOutput = target.GetType() != cmStateEnums::UTILITY &&
      target.GetType() != cmStateEnums::GLOBAL_TARGET;

    context.fc.WriteComment(std::string("Target definition: ") + targetName +
                            " (" + linkCommand + ")");
    context.fc.WritePushScope();

    std::vector<std::string> dependencies;
    std::vector<std::string> linkDependencies;
    Detection::DetectTargetCompileDependencies(
      context.self, target, dependencies,
      hasLinkerStage ? linkDependencies : dependencies);
    if (!dependencies.empty())
      context.fc.WriteComment(std::string("Target dependencies: ") +
                              Join(dependencies, " "));
    if (!linkDependencies.empty())
      context.fc.WriteComment(std::string("Target link dependencies: ") +
                              Join(linkDependencies, " "));

    // Output common config (for custom commands that do not depend on current
    // config)
    if (context.self->GetConfigurations().size() > 1) {
      ...
      // This is incase this target depends on code generated from previous
      // ones CHANGE: now we distinguish dependancies for link, so that compile
      // can be in parallel. Dependencies on generated codes should be explicit
      // based on the files themselves
      {
        context.fc.WriteArray("PreBuildDependencies",
    ...
    for (; configIter != configEnd; ++configIter) {
      ...
      if (hasOutput) {
        ...
        context.fc.WriteVariable("TargetNameOut",
                                 Quote(targetNames.targetName.Output));
        context.fc.WriteVariable("TargetNameImport",
                                 Quote(targetNames.targetName.ImportLibrary));
        context.fc.WriteVariable("TargetNamePDB",
                                 Quote(targetNames.targetName.PDB));
        context.fc.WriteVariable("TargetNameSO",
                                 Quote(targetNames.targetName.SharedObject));
        context.fc.WriteVariable("TargetNameReal",
                                 Quote(targetNames.targetName.Real));
    ...
    WriteCustomBuildSteps(context, lg, target, target.GetPreBuildCommands(),
                          "PreBuildCommands", dependencies);
    WriteCustomBuildSteps(context, lg, target, target.GetPreLinkCommands(),
                          "PreLinkCommands", dependencies);

    // Output the ExportAll (Prelink) command if WINDOWS_EXPORT_ALL_SYMBOLS is
    // on
    ...
    if (target.GetType() == cmStateEnums::SHARED_LIBRARY &&
        target.Target->GetMakefile()->IsOn(
    ...
    std::pair<bool, bool> hasCustomBuildRules =
      WriteCustomBuildRules(context, lg, target);
    ...
    // Add PreBuild per-config aliases to be able to add dependencies to all
    // prebuild steps of linked targets
    configIter = context.self->GetConfigurations().begin();
    configEnd = context.self->GetConfigurations().end();
    for (; configIter != configEnd; ++configIter) {
      const std::string& configName = *configIter;
      ...
      for (std::vector<std::string>::const_iterator it =
             linkDependencies.begin();
           it != linkDependencies.end(); ++it)
        preBuildTargets.push_back(*it + "-PreBuild-" + configName);
      for (std::vector<std::string>::const_iterator it = preBuildSteps.begin();
           it != preBuildSteps.end(); ++it)
      ...
      context.fc.WriteCommand("Alias",
                              Quote(targetName + "-PreBuild-" + configName));
      context.fc.WritePushScope();
      context.fc.WriteArray("Targets", Wrap(preBuildTargets, "'", "'"));
      context.fc.WritePopScope();
    ...
    if (!languages.empty()) {
      ...
      for (; configIter != configEnd; ++configIter) {
        ...
        if (hasPreBuildTargets) {
          std::vector<std::string> deps;
          deps.push_back(std::string("'") + targetName + "-PreBuild-" +
                         configName + "'");
    ...
    for (std::set<std::string>::iterator langIter = languages.begin();
      ...
      for (; configIter != configEnd; ++configIter) {
        const std::string& configName = *configIter;
        std::vector<std::string>& exportAllObjectFiles =
          exportAllConfigObjectFiles[configName];
        ...
        context.fc.WriteComment("Source files:");
        {
          ...
          for (std::vector<cmSourceFile const*>::iterator sourceIter =
            ...
            Detection::DetectCompilerFlags(compilerFlags, lg, target, srcFile,
                                           objectGroupLanguage, configName);
        ...
        for (std::map<std::string, CompileCommand>::iterator groupIter =
          ...
          context.fc.WriteVariable("CompilerOutputExtension",
                                   Quote(outputExtension));
          ...
          for (objectListIt = command.sourceFiles.begin();
            ...
            const std::string folderName(
              Detection::GetLastFolderName(sourceFile));

            // Ideally we need to take more than just the last folder name to
            // avoid conflicts with trees like aaa/item.cpp and
            // bbb/aaa/item.cpp which would both end in a aaa/item.cxx.obj file
            // (see for ex. the Qt*Autogen.Parallel* tests)
            //
            // For information, to avoid conflicts when using Ninja, unique
            // paths like this can be choosen: (comes from the the
            // Qt4Autogen.Parallel1 test)
            ...
            if (sourceFile.find(curSrcDir) != std::string::npos) {
              // Instead of the last folder name, take more parent folders to
              // avoid potential conflicts with other files with same name in
              // subdirectories
            ...
            */
            ...
            if (addExportAll && objectGroupLanguage != "RC") {
              for (std::vector<std::string>::const_iterator it =
                   ...
                   it != objectListIt->second.end(); ++it) {
                const std::string filename =
                  cmSystemTools::GetFilenameWithoutExtension(*it);
    ...
    if (addExportAll) {
      ...
      for (; configIter != configEnd; ++configIter) {
        const std::string& configName = *configIter;
        const std::vector<std::string>& exportAllObjectFiles =
          exportAllConfigObjectFiles[configName];
        ...
        for (std::vector<std::string>::const_iterator it =
               exportAllObjectFiles.begin();
             it != exportAllObjectFiles.end(); ++it) {
          fout << *it << "\n";
        }
      }
      ...
      std::vector<cmCustomCommand> commands;
      cmCustomCommand command(target.Target->GetMakefile(), outputs, empty,
                              empty, commandLines, "Auto build dll exports",
                              ".");
      ...
      WriteCustomBuildSteps(context, lg, target, commands, "ExportAll",
                            linkableDeps);
    }
      ...
      if (hasLinkerStage) {
        ...
        // Linker options
        {
          ...
          target.LocalGenerator->GetTargetFlags(
            linkLineComputer.get(), configName, linkLibs, targetFlags,
            linkFlags, frameworkPath, dummyLinkPath, &target);
          ...
          if (addExportAll || target.IsExecutableWithExports()) {
            const char* defFileFlag =
              target.Makefile->GetDefinition("CMAKE_LINK_DEF_FILE_FLAG");
          ...
          // Remove the command from the front and leave the flags behind
          std::string linkCmd;
          if (!Detection::DetectBaseLinkerCommand(linkCmd, lg, target,
                                                  configName)) {
            return;
          }
          ...
          std::transform(linkerType.begin(), linkerType.end(),
                         linkerType.begin(), ::tolower);
          ...
          // we must use the librarian (lib.exe) as link.exe won't accept /lib
          // in a response file. Fastbuild chooses to write a response file if
          // the arguments are too long:
          if (linkerType == "msvc" && linkCommand == "Library") {
            std::string flagsLower = flags;
            std::transform(flagsLower.begin(), flagsLower.end(),
                           flagsLower.begin(), ::tolower);
            ...
            if (found != std::string::npos)
              libExecutable =
                executable.replace(found, strlen("link.exe"), "lib.exe");
          }
          ...
          // Now detect the extra dependencies for linking
          {
            ...
            std::ostringstream log;
            Detection::DetectTargetLinkItems(target, configName,
                                             extraDependencies, log);
            ...
            Detection::DetectTargetLinkOtherInputs(lg, target, configName,
                                                   otherInputs);
            std::for_each(otherInputs.begin(), otherInputs.end(),
                          Detection::UnescapeFastbuildVariables);
          ...
          if (addExportAll) {
            std::vector<std::string> deps;
            deps.push_back(std::string("'") + targetName + "-ExportAll-" +
                           configName + "'");
            context.fc.WriteArray("PreBuildDependencies", deps, "+");
          }
          context.fc.WritePopScope();
                                 ...
                                 const std::vector<std::string>& targetDeps)
  {
    ...
    if (context.self->GetConfigurations().size() > 1) {
      {
        context.fc.WriteCommand("Alias", Quote(targetName + "-Common"));
        context.fc.WritePushScope();
        if (!commonDeps.empty()) {
          context.fc.WriteArray(
            "Targets", Wrap(commonDeps, "'" + targetName + "-", "-Common'"));
        } else {
          context.fc.WriteArray("Targets", Wrap(targetDeps, "'", "-Common'"));
        }
        context.fc.WritePopScope();
      }
    }
    ...
    for (; iter != end; ++iter) {
      ...
      // Always output targetName-Config alias for all configs, even when they
      // did not output anything
      // if (!orderDeps.empty() || !commonDeps.empty() ||
      // !linkableDeps.empty())
      {
        ...
        if (targetsOp == "=" && !targetDeps.empty()) {
          // no active output, directly refer to the cmake target dependencies
          // instead
          context.fc.WriteArray("Targets",
        ...
        if (targetsOp == "=") { // this target really is empty, but we still
                                // have to write Targets
          std::vector<std::string> empty;
          context.fc.WriteArray("Targets", empty, targetsOp);
        }
        context.fc.WritePopScope();
      }
    }
                                     ...
                                     bool outputGlobals)
  {
    ...
    for (OrderedTargets::iterator targetIter = context.orderedTargets.begin();
      ...
      // context.fc.WriteComment(std::string("Processing Target: ") +
      // constTarget->GetName() + " (" +
      // cmState::GetTargetTypeName(constTarget->GetType()) + ")");
      ...
      if (findResult == context.targetContexts.end()) {
        std::ostringstream s;
        s << "No TargetContext found for target " << constTarget->GetName()
          << " in " << constTarget->LocalGenerator->GetCurrentSourceDirectory()
          << " ( 0x" << std::hex << (unsigned long long)constTarget << std::dec
          << " ) "
          << " ( 0x" << std::hex << (unsigned long long)constTarget->Target
          << std::dec << " ) " << std::endl;
  ...
  static bool IsPartOfDefaultBuild(GenerationContext& context,
                                   cmGeneratorTarget* target,
                                   const std::string& configName)
  {
    if (!context.self->IsExcluded(target)) {
      return true;
    }
    if (target->GetType() == cmStateEnums::GLOBAL_TARGET) {
      ...
      if (target->GetName() == "INSTALL" || target->GetName() == "PACKAGE") {
        std::string definitionName =
          "CMAKE_VS_INCLUDE_" + target->GetName() + "_TO_DEFAULT_BUILD";
        const char* propertyValue =
          target->Target->GetMakefile()->GetDefinition(definitionName);
        cmGeneratorExpression ge;
        std::unique_ptr<cmCompiledGeneratorExpression> cge =
          ge.Parse(propertyValue);
        if (cmIsOn(cge->Evaluate(target->GetLocalGenerator(), configName))) {
          return true;
        }
      }
    }
  ...
  static void WriteAliases(GenerationContext& context, bool outputGlobals,
                           ...
                           std::string& finalTargetName)
  {
    context.fc.WriteSectionHeader(outputGlobals ? "Global Aliases"
                                                : "Target Aliases");
    ...
    context.fc.WriteSectionHeader(outputGlobals ? "Final Aliases"
                                                : "Config Aliases");
    ...
    if (!outputGlobals) {
      ...
      if (finalTargets.empty()) {
        finalTargetName = previousTargetName; // no new default target added,
                                              // keep the existing target
      } else {
        finalTargetName = "ALL";
        std::vector<std::string>::const_iterator iter = finalTargets.begin(),
  ...
  static void WriteVSConfigurations(GenerationContext& context)
  {
    ...
    // Translate the cmake compiler id into the PlatformName and
    // PlatformToolset
    cmMakefile* mf = context.root->GetMakefile();
    if (mf != 0) {
      ...
      std::string compilerVersion =
        mf->GetSafeDefinition(compilerVar + "_VERSION");
      if (compilerId == "MSVC") {
        ...
        if (mf->IsOn("CMAKE_CL_64")) {
          // context.fc.WriteComment("CMAKE_CL_64 is set, using x64
          // platform.");
    ...
    const std::map<std::string, std::string> configAlias =
      context.self->GetVSConfigAlias();
    ...
    for (; iter != end; ++iter) {
      ...
      std::map<std::string, std::string>::const_iterator vsconfigIter =
        configAlias.find(configName);
      const std::string& vsconfig =
        vsconfigIter == configAlias.end() ? configName : vsconfigIter->second;
      // Output platform (TODO: CURRENTLY ONLY HANDLED FOR WINDOWS)
      context.fc.WriteVariable("Platform",
                               Quote(context.self->GetPlatformName()));
      context.fc.WriteVariable("Config", Quote(vsconfig));
      context.fc.WriteVariable("ProjectBuildCommand",
                               Quote("cd ^$(SolutionDir) &amp; fbuild -vs "
                                     "-dist -cache -monitor ^$(ProjectName)-" +
                                     configName));
      context.fc.WriteVariable(
        "ProjectRebuildCommand",
        Quote("cd ^$(SolutionDir) &amp; fbuild -vs -dist -cache -monitor "
              "-clean ^$(ProjectName)-" +
              configName));
      // TODO: read this from cmake config...
      context.fc.WriteVariable("OutputDirectory",
                               Quote("^$(SolutionDir)bin/" + configName));
  ...
  static void WriteVSConfigAlias(GenerationContext& context,
  ...
  static void WriteVSProject(GenerationContext& context,
                             cmGeneratorTarget& target)
  {
    const std::string& targetName = target.GetName();
    ...
    context.fc.WriteCommand("VCXProject", Quote(targetName + "-project"));
    context.fc.WritePushScope();

    context.fc.WriteVariable(
      "ProjectOutput",
      Quote(target.GetSupportDirectory() + "/" + targetName + ".vcxproj"));
  ...
  static void WriteVSSolution(GenerationContext& context,
                              ...
                              const std::string& finalTargetName)
  {
    ...
    context.fc.WritePushScope();
    context.fc.WriteVariable(
      "ProjectOutput", Quote(rootDirectory + buildTargetName + ".vcxproj"));
    context.fc.WritePopScope();

    if (finalTargetName != buildTargetName) {
      // WriteVSConfigAlias(context, finalTargetName);
      context.fc.WriteCommand("VCXProject",
                              Quote(finalTargetName + "-project"));
      context.fc.WritePushScope();
      context.fc.WriteVariable(
        "ProjectOutput", Quote(rootDirectory + finalTargetName + ".vcxproj"));
      context.fc.WritePopScope();
    }
    ...
    topLevelSlnName +=
      (mf != 0) ? context.root->GetProjectName() : context.self->GetName();
    topLevelSlnName += "-FASTBuild.sln";

    context.fc.WriteVariable("SolutionOutput", Quote(topLevelSlnName));

    // context.fc.WriteArray("SolutionConfigs",
    ...
    if (useFolders)
      predefinedFolderName = context.self->GetPredefinedTargetsFolder();

    targetsInFolder[predefinedFolderName].push_back(
      Quote(buildTargetName + "-project"));
    if (finalTargetName != buildTargetName) {
      targetsInFolder[predefinedFolderName].push_back(
        Quote(finalTargetName + "-project"));
    }
    ...
    for (std::map<std::string, std::vector<std::string>>::const_iterator
           iter = targetsInFolder.begin(),
           itend = targetsInFolder.end();
         iter != itend; ++iter) {
    ...
    context.fc.WriteVariable("SolutionBuildProject",
                             Quote(finalTargetName + "-project"));
...
//----------------------------------------------------------------------------
bool cmGlobalFastbuildGenerator::SetGeneratorPlatform(std::string const& p,
                                                      cmMakefile* mf)
{
  if (p == "Win32" || p == "x64" || p == "Itanium" || p == "ARM" || p == "") {
    this->GeneratorPlatform = p;
  } else {
    return this->cmGlobalGenerator::SetGeneratorPlatform(
      p, mf); // unrecognized platform
  }
...
//----------------------------------------------------------------------------
auto cmGlobalFastbuildGenerator::GenerateBuildCommand(
  const std::string& makeProgram, const std::string& projectName,
  const std::string& projectDir, std::vector<std::string> const& targetNames,
  const std::string& config, bool /*fast*/, int jobs, bool /*verbose*/,
  std::vector<std::string> const& makeOptions)
  -> std::vector<GeneratedMakeCommand>
{
  ...
  // Select the target
  std::vector<std::string> targetsSelected;
  ...
  for (const std::string& target : targetNames) {
    ...
    targetsSelected.push_back(target + "-" + configSelected);
  }
  if (targetsSelected.empty()) {
    targetsSelected.push_back(configSelected);
  }
  ...
  makeCommand.Add(makeOptionsSelected.cbegin(), makeOptionsSelected.cend());
  ...
  if (!configFile.empty()) {
    ...
    makeCommand.Add(configFile);
  }

  // Add the selected target-config to the command
  makeCommand.Add(targetsSelected.cbegin(), targetsSelected.cend());
  return { makeCommand };
}
...
cmGlobalFastbuildGenerator::GetVSConfigAlias() const
{
...
std::string cmGlobalFastbuildGenerator::ConvertToFastbuildPath(
  const std::string& path)
{
  return LocalGenerators[0]->MaybeConvertToRelativePath(
